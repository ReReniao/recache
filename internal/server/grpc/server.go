...
type Server struct {
	...
	stopSignal     chan error // registry revoke
	mu             sync.Mutex
...
func (s *Server) Get(ctx context.Context, req *pb.GetRequest) (*pb.GetResponse, error) {
	...
	resp := &pb.GetResponse{}
	logger.Logger.Infof("[recache server %s] Recv RPC Request - (%s)/(%s)", s.Addr, groupName, key)
...
func (s *Server) UpdatePeers(peerAddrs []string) {
	...
	s.consistenthash = consistenthash.NewMap(defaultReplicas, nil)
	s.consistenthash.AddNode(peerAddrs...)
	...
	for _, peerAddr := range peerAddrs {
		...
		serviceNameWithPrefix := fmt.Sprintf("recache/%s", peerAddr)
...
func (s *Server) reconstruct() {
	serviceList := etcdservice.ListServicePeers("recache")
	...
	s.mu.Lock()
	s.consistenthash = consistenthash.NewMap(defaultReplicas, nil)
	s.consistenthash.AddNode(serviceList...)
	s.fetchers = make(map[string]*grpcFetcher)
